...
export interface EventOption {
  ...
  // Le résolveur renvoie un message de log décrivant ce qui s'est passé.
  resolve: (run: RunState, rand: () => number) => string;
}
...
const addGold = (run: RunState, n: number) => { run.gold = Math.max(0, run.gold + n); };
const hurt = (run: RunState, n: number) => { run.hp = Math.max(1, run.hp - n); };
const heal = (run: RunState, n: number) => { run.hp = Math.min(run.maxHp, run.hp + n); };
const addCard = (run: RunState, defId: string, upgraded = false) => {
  run.deck.push({ uid: `ev_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`, defId, upgraded });
};
const removeRandomCard = (run: RunState, rand: () => number) => {
...
export const ALL_EVENTS: GameEvent[] = [
  {
    ...
    body:
      'Une idole d\u2019os trempe dans une flaque de ses propres larmes. Un murmure promet la force à qui prêtera un faux serment.',
    options: [
      {
...

  {
    ...
    options: [
      ...
      },
      {
        ...
        resolve: (run) => {
          run.maxHp += 2; heal(run, 2); addGold(run, 10);
    ...
    options: [
      {
        ...
        resolve: (run, rand) => {
          ...
          if (!eligible.length) return 'Aucune copie n\u2019apparaît.';
