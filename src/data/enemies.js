...
const NORMAL = [
    ...
    def({
        ...
        pickMove: (ctx) => {
            if (ctx.lastMove === 'bite' && ctx.rng() < 0.5)
                return 'scurry';
            return 'bite';
        },
    }),
...
const ELITE = [
    ...
    def({
        ...
        pickMove: (ctx) => {
            ...
            if (ctx.turn === 1)
                return 'peal';
            if (ctx.lastMove === 'toll')
                return r < 0.5 ? 'clangor' : 'peal';
            if (ctx.lastMove === 'clangor')
                return 'toll';
            return 'toll';
        },
    }),
    def({
        ...
        pickMove: (ctx) => {
            ...
            if (hist.endsWith('maw'))
                return 'echo';
            if (hist.endsWith('echo'))
                return 'maw';
...
const BOSSES = [
    def({
        ...
        pickMove: (ctx) => {
            ...
            if (ctx.hpPct < 0.5) {
                if (ctx.lastMove === 'finale')
                    return 'crescendo';
                if (ctx.lastMove === 'crescendo')
                    return 'antiphon';
                return 'finale';
            }
...
export const ALL_ENEMIES = [...NORMAL, ...ELITE, ...BOSSES];
export const ENEMY_MAP = Object.fromEntries(ALL_ENEMIES.map((e) => [e.id, e]));
