...
function getStatus(c, id) {
    return c.status[id] ?? 0;
}
/** Apply the same per-hit mod chain as combat.ts applyAttack. */
function modifyDamage(base, source, target) {
...
export function previewCardDamage(def, source, target) {
    let total = 0;
    let hits = 0;
    let lastPerHit = 0;
    function walk(ops, mult = 1) {
        for (const op of ops) {
            if (op.kind === 'damage') {
...
export function previewIncomingDamage(source, target, intent) {
