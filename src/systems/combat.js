...
import { makeRng, pickN, randInt, shuffle as shuffleRng, makeUid } from './rng';
...
function float(state, targetId, text, tone, delayMs = 0, source) {
    state.pendingFloat.push({ targetId, text, tone, delayMs, source });
}
function shake(state, targetId, amount = 1) {
...
function setStatus(target, id, amount) {
    if (amount <= 0)
        ...
        target.status[id] = amount;
}
function clearStatus(target, id) {
    delete target.status[id];
}
function applyDebuff(state, target, id, amount) {
    if (amount <= 0)
        return;
...
function summonServants(state, amount) {
    if (amount <= 0)
        return;
    state.summons ??= [];
...
function commandSummons(state, rng, damage, target, targetMode = 'enemy', consume) {
    ...
    if (consume != null) {
        for (const s of servants.slice(0, strikes))
            s.hp = 0;
        syncSummonStatus(state);
    }
}
function triggerSummonTurnStart(state, rng, relics) {
    const servants = livingSummons(state);
    if (!servants.length)
        return;
...
export function buildCombat(opts) {
    const rng = makeRng(opts.seed);
    const enemies = opts.encounter.map((defId, i) => {
    ...
    const player = {
        id: 'player',
        name: opts.classId === 'vowbreaker' ? 'Vowbreaker' :
    ...
    const draw = shuffleRng(rng, opts.deck);
    const state = {
        turn: 0,
        phase: 'player',
        player,
...
// ---------------- turn flow ----------------
export function beginPlayerTurn(state, rng, relics) {
    state.phase = 'player';
    // reset ember
...
export function endPlayerTurn(state, rng, relics) {
    state.phase = 'enemy';
...
function clearPerTurnFlags(state) {
    ...
    delete state.player.status._firstAttack;
    delete state.player.status._firstSkill;
    delete state.player.status._firstSeal;
    delete state.player.status._first_upgraded_discount;
    delete state.player.status._potion_refilled;
    delete state.player.status._first_bleed_doubled;
}
function decayStatuses(c) {
    for (const id of Object.keys(c.status)) {
...
function handleOnDrawCurses(state) {
    for (const c of state.hand) {
        const def = getCard(c.defId);
...
export function banishCard(state, card) {
...
// ---------------- damage ----------------
export function applyDirectDamage(state, target, amount, delayMs = 0, source) {
    if (amount <= 0)
        return;
...
export function applyAttack(ctx, baseAmount) {
    ...
    if (getStatus(source, 'faded') > 0)
        amt = Math.floor(amt * 0.75);
    // Brittle on target: +50%
    if (getStatus(target, 'brittle') > 0)
        amt = Math.floor(amt * 1.5);
    // Relic: Shard of First Vow â€” first attack each turn +2
    if (source.isPlayer && cardDef?.type === 'attack' && !source.status._firstAttack) {
        if (relics.includes('rel_shard_of_first_vow'))
            amt += 2;
        source.status._firstAttack = 1;
        // Rust Medallion: first attack each turn apply 1 Brittle
        if (relics.includes('rel_rust_medallion'))
            applyDebuff(state, target, 'brittle', 1);
    }
    ...
    if (source.isPlayer && !target.isPlayer) {
        ...
        if (thorns > 0)
            applyDirectDamage(state, source, thorns);
    }
...
// ---------------- Op runner ----------------
export function playCard(state, rng, relics, cardUid, targetId, xEmber) {
    const idx = state.hand.findIndex((c) => c.uid === cardUid);
    if (idx < 0)
        return { played: false, reason: 'not in hand' };
    const ci = state.hand[idx];
    const def = getCard(ci.defId);
    // Unplayable curse or status
    if (def.type === 'curse')
        return { played: false, reason: 'unplayable' };
    if (def.id === 'status_dross')
        return { played: false, reason: 'unplayable' };
    // Chained forbids Ward gain; otherwise not blocking play.
    // Cost handling
    let cost = def.cost;
    if (cost < 0)
        cost = xEmber ?? state.ember;
    // Upgraded card discount relic
    if (ci.upgraded && relics.includes('rel_forged_sigil') && !state.player.status._first_upgraded_discount) {
        cost = Math.max(0, cost - 1);
        state.player.status._first_upgraded_discount = 1;
    }
    if (cost > state.ember)
        return { played: false, reason: 'not enough ember' };
    // Oathlight flag discount (all Uncommon Sealbinder skills this turn cost 0)
    if (state.player.status._oathlight && def.classId === 'sealbinder' && def.type === 'skill' && def.rarity === 'uncommon') {
        cost = 0;
    }
    // Pay
    state.ember -= cost;
    ...
    if (def.type === 'skill') {
        if (!state.player.status._firstSkill) {
            state.player.status._firstSkill = 1;
            if (relics.includes('rel_widow_stone'))
...
function runCardTags(state, rng, relics, def, ci, costPaid, target) {
    if (!def.tags?.length)
        return;
    for (const tag of def.tags) {
        switch (tag) {
            ...
            case 'x:fracture_unleashed_p': {
                ...
                if (target)
                    applyAttack({ source: state.player, target, state, relics, cardDef: def }, per * stacks);
            ...
            case 'x:oathlight_p': {
                ...
                for (const p of picks)
                    state.hand.push({ uid: makeUid('oathlight'), defId: p.id, upgraded: false });
                state.player.status._oathlight = 1;
                break;
            }
            default:
...
function runOps(state, rng, relics, ops, ctx) {
    for (const op of ops)
        runOp(state, rng, relics, op, ctx);
}
function runOp(state, rng, relics, op, ctx) {
    switch (op.kind) {
        case 'damage': {
        ...
        case 'status': {
            ...
            for (const t of targets) {
                ...
                if (isDebuff)
                    applyDebuff(state, t, op.id, op.amount);
                else {
                    addStatus(t, op.id, op.amount);
                    float(state, t.id, `+${op.amount} ${statusLabel(op.id)}`, 'status');
                    if (op.id === 'seal' && !t.isPlayer)
                        onPlayerAppliedSeal(state, relics, op.amount);
                    // First-bleed doubled relic
                    if (op.id === 'bleed' && !t.isPlayer && relics.includes('rel_ember_thorn') && !state.player.status._first_bleed_doubled) {
                        addStatus(t, 'bleed', op.amount);
                        state.player.status._first_bleed_doubled = 1;
        ...
        case 'draw':
            drawCards(state, op.amount, rng);
            break;
        case 'gain_ember':
            state.ember += op.amount;
            break;
        case 'lose_hp':
            applyDirectHpLoss(state, ctx.source, op.amount, relics);
            break;
        case 'heal':
            ctx.source.hp = Math.min(ctx.source.maxHp, ctx.source.hp + op.amount);
            float(state, ctx.source.id, `+${op.amount}`, 'heal');
            break;
        case 'discard_random': {
        ...
        case 'add_card_to_hand': {
            ...
            for (let i = 0; i < count; i++) {
                if (state.hand.length >= 10)
                    state.discard.push({ uid: makeUid('add'), defId: op.cardId, upgraded: false });
                else
                    state.hand.push({ uid: makeUid('add'), defId: op.cardId, upgraded: false });
            }
            break;
        }
        case 'add_card_to_discard': {
            const count = op.amount ?? 1;
            for (let i = 0; i < count; i++)
                state.discard.push({ uid: makeUid('addd'), defId: op.cardId, upgraded: false });
            break;
        }
        case 'summon': {
        ...
        case 'double_status': {
            ...
            for (const t of targets) {
                ...
                if (v > 0)
                    setStatus(t, op.id, v * 2);
            }
            break;
        }
        case 'if_has_status': {
            const tgt = resolveTargets(state, ctx.source, op.target, ctx.target?.id)[0];
            if (tgt && getStatus(tgt, op.id) > 0)
                runOps(state, rng, relics, op.then, ctx);
            else if (op.else)
                runOps(state, rng, relics, op.else, ctx);
            break;
        }
        case 'if_hp_below': {
        ...
        case 'repeat': {
            for (let i = 0; i < op.times; i++)
                runOps(state, rng, relics, op.ops, ctx);
            break;
        }
        case 'per_target_status': {
            ...
            for (const t of targets) {
                ...
                for (let i = 0; i < stacks; i++)
                    runOp(state, rng, relics, { ...op.then, target: t.isPlayer ? 'self' : 'enemy' }, { ...ctx, target: t });
            }
            break;
        }
        case 'spend_fracture': {
...
// ---------------- ward / hp helpers ----------------
function gainWard(state, target, amount, relics) {
    if (amount <= 0)
        return;
    ...
    if (target.isPlayer)
        amount += getStatus(target, 'poise');
    target.ward += amount;
    float(state, target.id, `+${amount} garde`, 'ward');
}
function applyDirectHpLoss(state, target, amount, relics) {
    target.hp = Math.max(0, target.hp - amount);
    float(state, target.id, `-${amount}`, 'dmg');
    if (target.isPlayer)
        onPlayerLoseHp(state, relics);
}
function onPlayerLoseHp(state, relics) {
    if (hasPowerTag(state, 'hook:lose_hp_gain_fracture_2'))
        addStatus(state.player, 'fracture', 2);
    else if (hasPowerTag(state, 'hook:lose_hp_gain_fracture'))
        addStatus(state.player, 'fracture', 1);
}
export function applyRelicHook(state, hook, ctx) {
    switch (hook) {
        case 'start_combat_draw_1':
            if (ctx.phase === 'combat_start')
                state.drawPerTurn += 1;
            break;
        case 'start_combat_add_spark':
            if (ctx.phase === 'combat_start')
                state.hand.push({ uid: makeUid('spark'), defId: 'status_spark', upgraded: false });
            break;
        case 'start_combat_summon_1':
            if (ctx.phase === 'combat_start')
                summonServants(state, 1);
            break;
        case 'start_combat_2_ward':
            if (ctx.phase === 'combat_start')
                gainWard(state, state.player, 2, ctx.relics);
            break;
        case 'start_combat_3_ward':
            if (ctx.phase === 'combat_start')
                gainWard(state, state.player, 3, ctx.relics);
            break;
        ...
        case 'at_turn_start_if_hand_5_plus_gain_1_momentum':
            if (ctx.phase === 'turn_start' && state.hand.length >= 5)
                addStatus(state.player, 'momentum', 1);
            break;
        case 'start_of_turn_gain_1_ember_if_0_cards_in_hand':
            if (ctx.phase === 'turn_start' && state.hand.length === 0)
                state.ember += 1;
            break;
...
function applyPowerHooks(state, phase, rng, relics) {
    for (const ci of state.banish) {
        ...
        for (const tag of def.tags) {
            if (phase === 'turn_start') {
                switch (tag) {
                    case 'hook:turn_start_gain_fracture_1':
                        addStatus(state.player, 'fracture', 1);
                        break;
                    case 'hook:turn_start_gain_fracture_2':
                        addStatus(state.player, 'fracture', 2);
                        break;
                    case 'hook:turn_start_ward_4':
                        gainWard(state, state.player, 4, relics);
                        break;
                    case 'hook:turn_start_ward_6':
                        gainWard(state, state.player, 6, relics);
                        break;
...
function hasPowerTag(state, tag) {
    for (const ci of state.banish) {
        const def = getCard(ci.defId);
        if (def.type === 'power' && def.tags?.includes(tag))
            return true;
    }
    return false;
}
function onPlayerAppliedSeal(state, relics, amount) {
    // Starter relic: Gavel of Judgement
    if (relics.includes('rel_gavel_of_judgement') && !state.player.status._firstSeal) {
        state.player.status._firstSeal = 1;
        gainWard(state, state.player, 2, relics);
    }
    // Power cards reacting to seal application
    if (hasPowerTag(state, 'hook:seal_gain_ward_2'))
        gainWard(state, state.player, 2 * amount, relics);
    else if (hasPowerTag(state, 'hook:seal_gain_ward_1'))
        gainWard(state, state.player, 1 * amount, relics);
}
function onPlayerAppliedDebuff(state, _id) {
...
function onEnemyDeath(state, enemy, relics) {
    // Sanguine Rite hook
...
// ---------------- enemy turn ----------------
function enemyTurn(state, rng, relics) {
    for (const e of state.enemies) {
        if (e.hp <= 0)
            continue;
...
function executeEnemyMove(state, e, rng, relics) {
...
function pickNextIntents(state, rng, _relics) {
    for (const e of state.enemies) {
        if (e.hp <= 0)
            continue;
        const def = getEnemy(e.defId);
...
function resolveTarget(state, source, mode, id) {
    if (mode === 'none')
        return null;
    if (mode === 'self')
        return source;
    if (mode === 'enemy') {
        if (source.isPlayer) {
            if (id && id !== 'player')
                return state.enemies.find((e) => e.id === id) ?? state.enemies.find((e) => e.hp > 0) ?? null;
...
function resolveTargets(state, source, mode, id) {
    if (mode === 'all_enemies') {
        if (source.isPlayer)
            return state.enemies.filter((e) => e.hp > 0);
...
function allEnemiesDead(state) { return state.enemies.every((e) => e.hp <= 0); }
function cleanupDead(state, relics) {
...
export function usePotion(state, rng, relics, potionId, targetId) {
...
function usePotionInternal(state, rng, relics, potionId, targetId) {
    const p = getPotion(potionId);
    const target = targetId ? state.enemies.find((e) => e.id === targetId) ?? null : null;
    switch (p.apply.kind) {
        case 'heal':
            state.player.hp = Math.min(state.player.maxHp, state.player.hp + p.apply.amount);
            float(state, 'player', `+${p.apply.amount}`, 'heal');
            break;
        case 'ember':
            state.ember += p.apply.amount;
            break;
        case 'draw':
            drawCards(state, p.apply.amount, rng);
            break;
        case 'damage':
            if (target)
                applyAttack({ source: state.player, target, state, relics }, p.apply.amount);
            break;
        case 'ward':
            gainWard(state, state.player, p.apply.amount, relics);
            break;
        case 'status_self':
            addStatus(state.player, p.apply.id, p.apply.amount);
            break;
        case 'status_enemy':
            if (target)
                applyDebuff(state, target, p.apply.id, p.apply.amount);
            break;
        case 'cleanse':
            for (const id of Object.keys(state.player.status))
                if (STATUS_IS_DEBUFF[id])
                    clearStatus(state.player, id);
            break;
