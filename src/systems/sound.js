...
function impulse(ac, seconds = 2.2, decay = 3.6) {
...
function connect(node, bus = 'sfx', wet = 0) {
    const ac = getCtx();
    if (!ac || !graph)
        return;
...
function env(gain, when, peak, attack, hold, release = 0.0001) {
...
function chord(freqs, duration, gain, delay = 0, wet = 0.12, bus = 'sfx') {
    const ac = getCtx();
    if (!ac)
        return;
...
function emberKick(delay = 0, heavy = false, bus = 'sfx') {
    const ac = getCtx();
    if (!ac)
        return;
...
function bell(root, delay = 0, gain = 0.08, bus = 'sfx') {
    const ac = getCtx();
    if (!ac)
        return;
    const t = ac.currentTime + delay;
...
function scheduleMenuStep(step, t) {
    ...
    if (pulse === 10 || pulse === 24)
        bell(root * 6, t - (getCtx()?.currentTime ?? t), 0.018, 'music');
    if (pulse === 16)
        noise({ duration: 1.2, gain: 0.018, filterStart: 1100, filterEnd: 90, type: 'bandpass', when: t, bus: 'music', wet: 0.36 });
}
function scheduleMapStep(step, t) {
    ...
    if (pulse === 15)
        noise({ duration: 0.36, gain: 0.032, filterStart: 2200, filterEnd: 5200, type: 'highpass', when: t, bus: 'music', wet: 0.24 });
}
function scheduleSanctuaryStep(step, t) {
    ...
    if (pulse === 2 || pulse === 8 || pulse === 16)
        bell(root * 8, t - (getCtx()?.currentTime ?? t), 0.034, 'music');
    if (pulse === 12)
        chord([root * 2, root * 2.5, root * 3, root * 4], 1.8, 0.06, t - (getCtx()?.currentTime ?? t), 0.42, 'music');
    if (pulse === 20)
        noise({ duration: 0.34, gain: 0.018, filterStart: 7800, filterEnd: 3200, type: 'highpass', when: t, bus: 'music', wet: 0.34 });
}
function scheduleEventStep(step, t) {
    ...
    if (pulse === 1 || pulse === 5 || pulse === 9)
        noise({ duration: 0.13, gain: 0.042, filterStart: 5200, filterEnd: 520, type: 'bandpass', q: 2.8, when: t, bus: 'music', wet: 0.32 });
    if (pulse === 6)
        bell(root * 7.1, t - (getCtx()?.currentTime ?? t), 0.028, 'music');
    if (pulse === 11)
        synth({ freq: root * 5.6, duration: 0.26, gain: 0.032, type: 'sawtooth', filterStart: 900, filterEnd: 5200, when: t, attack: 0.01, pan: 0.2, wet: 0.36 });
}
function scheduleRewardStep(step, t) {
    const bar = Math.floor(step / 16);
    const root = [82.41, 98, 110, 130.81][bar % 4];
    const pulse = step % 16;
    if (pulse === 0)
        chord([root * 2, root * 2.5, root * 3, root * 4, root * 6], 2.4, 0.095, t - (getCtx()?.currentTime ?? t), 0.4, 'music');
    if (pulse === 0 || pulse === 4 || pulse === 8 || pulse === 12)
        bell(root * (pulse === 8 ? 10 : 8), t - (getCtx()?.currentTime ?? t), 0.043, 'music');
    if (pulse === 2 || pulse === 6 || pulse === 10 || pulse === 14)
        synth({ freq: root * [3, 4, 5, 6][Math.floor(pulse / 4)], duration: 0.2, gain: 0.042, type: 'triangle', filterStart: 1800, filterEnd: 5200, when: t, wet: 0.26 });
}
function scheduleEndStep(step, t, victory) {
    const bar = Math.floor(step / 16);
...
function scheduleCombatStep(step, t) {
    const bar = Math.floor(step / 16);
...
function scheduleMusicStep(step, t) {
    if (currentMood === 'menu')
        scheduleMenuStep(step, t);
    else if (currentMood === 'map')
        scheduleMapStep(step, t);
    else if (currentMood === 'sanctuary')
        scheduleSanctuaryStep(step, t);
    else if (currentMood === 'event')
        scheduleEventStep(step, t);
    else if (currentMood === 'reward')
        scheduleRewardStep(step, t);
    else if (currentMood === 'victory')
        scheduleEndStep(step, t, true);
    else if (currentMood === 'defeat')
        scheduleEndStep(step, t, false);
    else
        scheduleCombatStep(step, t);
}
function moodLevel(mood) {
    if (mood === 'combat')
        return 0.56;
    if (mood === 'victory')
        return 0.52;
...
export function setAmbienceMode(mood) {
    if (currentMood === mood)
        return;
    currentMood = mood;
    musicStep = 0;
...
export function startAmbience(mood = 'menu') {
    currentMood = mood;
    if (ambience)
        return;
    const ac = getCtx();
