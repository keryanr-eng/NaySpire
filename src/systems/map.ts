...
import type { MapNode, NodeKind, RunMap } from '../types';
import { makeRng, mix, pick, Rng, weightedPick } from './rng';
...
export function generateMap(seed: number): RunMap {
  ...
  for (let f = 0; f < FLOORS; f++) {
    ...
    const floor: MapNode[] = cols.map((col, i) => ({
      id: `n_${f}_${col}_${i}`,
      floor: f,
      col,
      kind: 'combat',
  ...
  for (const n of floors[FLOORS - 1]) n.children.push(boss.id);

  // 5) Bake encounter/event ids deterministically per node
  const nodes: Record<string, MapNode> = {};
  for (const f of floors) for (const n of f) nodes[n.id] = n;
  nodes[boss.id] = boss;
...
function chooseCols(rng: Rng, count: number): number[] {
...
function rollKindForFloor(rng: Rng, f: number): NodeKind {
  if (f === 0) return weightedPick(rng, [['combat', 80], ['event', 20]] as [NodeKind, number][]);
  if (f === 1) return weightedPick(rng, [['combat', 70], ['event', 20], ['rest', 10]] as [NodeKind, number][]);
  // Mid-act: more variety, elites appear
  if (f < 6) return weightedPick(rng, [
    ['combat', 50], ['event', 18], ['elite', 10], ['merchant', 10], ['rest', 8], ['treasure', 4],
  ] as [NodeKind, number][]);
  // Late act
  if (f < FLOORS - 2) return weightedPick(rng, [
...
function ensureVariety(floor: MapNode[], _rng: Rng) {
...
function ensurePreBossRest(lastRow: MapNode[], rng: Rng) {
  if (!lastRow.some((n) => n.kind === 'rest')) {
    pick(rng, lastRow).kind = 'rest';
  }
}

function connectFloors(a: MapNode[], b: MapNode[], rng: Rng) {
  ...
  for (const dst of b) {
    ...
    if (!nearest.children.includes(dst.id)) nearest.children.push(dst.id);
  }
  // Dedup children
  for (const src of a) src.children = Array.from(new Set(src.children));
}

function bakeContent(nodes: Record<string, MapNode>, _seed: number) {
...
export function serializeEncounter(enemies: string[]): string { return enemies.join('|'); }
export function parseEncounter(s: string): string[] { return s.split('|'); }

// Utility: what nodes can the player choose NEXT given currentId?
export function nextChoices(map: RunMap, currentId: string | null): MapNode[] {
  if (currentId === null) return map.startIds.map((id) => map.nodes[id]);
  const cur = map.nodes[currentId];
  return cur.children.map((id) => map.nodes[id]);
}

// Sanity check helpers (used by tests/dev)
export function mapStats(map: RunMap) {
  const counts: Record<string, number> = {};
  for (const id in map.nodes) counts[map.nodes[id].kind] = (counts[map.nodes[id].kind] ?? 0) + 1;
  return counts;
}
