...
import { getCard, rewardPool, startingDeck } from '../data/cards';
import { ALL_RELICS, getRelic, relicRewardPool } from '../data/relics';
import { ALL_POTIONS, potionRewardPool } from '../data/potions';
import { generateMap, nextChoices, parseEncounter } from '../systems/map';
import { getEvent } from '../data/events';
import { buildCombat, endPlayerTurn, playCard, usePotion, } from '../systems/combat';
import { makeRng, mix, pickN, randInt } from '../systems/rng';
// ---------- save keys ----------
const SAVE_KEY_RUN = 'embervow.run.v1';
const SAVE_KEY_META = 'embervow.meta.v1';
// ---------- helpers ----------
function newCardInstance(defId, upgraded = false) {
    return { uid: `c_${Date.now()}_${Math.random().toString(36).slice(2, 8)}_${Math.floor(Math.random() * 1e6).toString(36)}`, defId, upgraded };
}
function startingMaxHp(classId) {
    if (classId === 'vowbreaker')
        return 75;
    if (classId === 'sealbinder')
        return 65;
    if (classId === 'whisperer')
        return 62;
    if (classId === 'summoner')
        return 58;
    return 60;
}
function starterRelicId(classId) {
    if (classId === 'vowbreaker')
        return 'rel_shard_of_first_vow';
    if (classId === 'sealbinder')
        return 'rel_gavel_of_judgement';
    if (classId === 'whisperer')
        return 'rel_censer_of_echoes';
    if (classId === 'summoner')
        return 'rel_bell_of_the_bound';
    return 'rel_oracle_lens';
}
function makeStartingDeck(classId) {
...
function goldForCombat(tier, rng, run) {
...
export const useGame = create((set, get) => ({
    ...
    newRun: (classId, seed) => {
        ...
        const run = {
            ...
            seen: { enemies: [], events: [] },
            flags: {},
            history: [`Tentative lancée avec ${classId === 'vowbreaker' ? 'Vowbreaker' :
                    classId === 'sealbinder' ? 'Sealbinder' :
                        classId === 'whisperer' ? 'Whisperer' :
                            classId === 'summoner' ? 'Summoner' : 'Auger'} (seed ${s}).`],
        };
        // Apply max-HP relic effect (doesn't appear in starter but future-proof)
        if (run.relics.includes('rel_ashworn_ring')) {
            run.maxHp += 7;
            run.hp += 7;
        }
        saveRun(run);
    ...
    startCombatForNode: () => {
        ...
        const tier = node.kind === 'boss' ? 'boss' : node.kind === 'elite' ? 'elite' : 'normal';
        const combat = buildCombat({
            ...
            deck: run.deck.map((c) => ({ uid: c.uid, defId: getCard(c.defId).upgradeTo && c.upgraded ? getCard(c.defId).upgradeTo : c.defId, upgraded: c.upgraded })),
            classId: run.classId,
            tier,
        });
    ...
    combatUsePotion: (slot, targetId) => {
        ...
        if (run.relics.includes('rel_wick_flask') && !combat.player.status._potion_refilled) {
            combat.player.status._potion_refilled = 1;
        }
        else {
    ...
    combatFinish: () => {
        ...
        if (run.relics.includes('rel_cinder_cloak'))
        ...
        if (current?.kind === 'boss' && run.relics.includes('rel_reclaimed_vow'))
            run.hp = run.maxHp;
        // Track seen
        for (const e of combat.enemies.map((x) => x.defId))
            if (!run.seen.enemies.includes(e))
                run.seen.enemies.push(e);
        // Compose reward
        const rng = makeRng(mix(run.seed, combat.turn, 31));
        const tier = current?.kind === 'boss' ? 'boss' : current?.kind === 'elite' ? 'elite' : 'normal';
        ...
        if (potion)
            pending.push('potion');
        const rewards = { gold, cards: rolls, relic, potion, pending };
    ...
    takeReward: (type, payload) => {
        ...
        if (type === 'relic' && rewards.relic) {
            ...
            if (relicDef.hook === 'max_hp_plus_7') {
                run.maxHp += 7;
                run.hp += 7;
            }
    ...
    rest: (choice) => {
        ...
        if (choice === 'heal') {
            ...
            if (run.relics.includes('rel_hearth_of_sanctum'))
                heal += 10;
            if (run.flags.full_heal_next_rest) {
                run.hp = run.maxHp;
                delete run.flags.full_heal_next_rest;
            }
            else
...
function priceFor(rarity, rng) {
    if (rarity === 'common')
        return 45 + Math.floor(rng() * 10);
    if (rarity === 'uncommon')
        return 70 + Math.floor(rng() * 15);
    if (rarity === 'rare')
        return 130 + Math.floor(rng() * 20);
    return 60;
}
function priceForRelic(tier, rng) {
    if (tier === 'common')
        return 140 + Math.floor(rng() * 20);
    if (tier === 'uncommon')
        return 200 + Math.floor(rng() * 25);
    if (tier === 'rare')
        return 260 + Math.floor(rng() * 40);
    return 180;
}
function priceForPotion(rarity, rng) {
    if (rarity === 'common')
        return 50 + Math.floor(rng() * 10);
    if (rarity === 'uncommon')
        return 70 + Math.floor(rng() * 10);
